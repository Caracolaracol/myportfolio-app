...
import Sliders from '@/components/ProjectSection/Slider'
...
function WebsiteProject({projectData}:any) {
    ...
    <div className="flex flex-col   tablet:min-h-[21rem] laptop:min-h-[12rem] bg-blancod bg-opacity-40 dark:bg-negron dark:bg-opacity-90 rounded-tr-sm rounded-br-sm p-4 tablet:pb-8">
        <div key={projectData.name} className="desktop:min-h-[9rem] transitionshort laptop:px-[5%] desktop:px-[2vw] pb-4">
            <div className="text-left">
                {projectData.illustrations ? <Image alt="ilustration" width='640' height='640' src={`/assets${projectData.illustrations}`} className='w-40 tablet:w-[18rem] float-right ml-3  tablet:ml-4 svg mr-4 tablet:mr-5' /> : ''}
            </div>
